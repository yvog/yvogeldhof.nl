...
export const ArrayFacetFilter = ({ id, title, type, posts }: ArrayFacetFilterProps) => {
  const { filters, setFilters } = useFilterContext();
  const { asPath, replace: replaceRoute } = useRouter();

  const onCheckboxFacetFilterChange = useCallback(
    (type: FacetFilterType, checked: boolean, filterValue: string) => {
      ...
      const search = toggleURLSearchParam(
        type,
        newState.join(','),
        newState.length > 0
      );
