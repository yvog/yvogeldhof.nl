...
By setting this flag, we are able to add the web part to a Full Width Column and the React web part will become mobile responsive as well. We created the React app using the SPFx framework, because we need a .sppkg file to deploy the web part to a SharePoint tenant's site collection app catalog.
